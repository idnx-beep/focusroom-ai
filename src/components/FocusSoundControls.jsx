...
export function FocusSoundControls({ scene, sound, setSound, isAudioOn, onPreviewAudio, onStopAudio }) {
  ...
  return (
    <div className="grid min-w-0 gap-2 sm:grid-cols-2 lg:contents">
      <section className="min-w-0 rounded-2xl border border-white/12 bg-black/20 p-3 sm:rounded-3xl sm:p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={isAudioOn ? onStopAudio : onPreviewAudio} className="focus-button h-11 w-11 shrink-0 p-0" aria-label={isAudioOn ? "停止声音" : "播放声音"}>
            {isAudioOn ? <Pause size={15} /> : <Play size={15} />}
          </button>
        </div>
        <select value={currentMusic.id} onChange={(event) => setType("musicType", event.target.value)} aria-label="切换音乐类型" className="mt-3 min-h-11 w-full rounded-2xl border border-white/12 bg-slate-950/70 px-3 py-2 text-sm text-white outline-none focus:border-sky-200/70">
      ...
      <section className="min-w-0 rounded-2xl border border-white/12 bg-black/20 p-3 sm:rounded-3xl sm:p-4">
        ...
        <select value={currentAmbience.id} onChange={(event) => setType("ambienceType", event.target.value)} aria-label="切换背景音类型" className="mt-3 min-h-11 w-full rounded-2xl border border-white/12 bg-slate-950/70 px-3 py-2 text-sm text-white outline-none focus:border-sky-200/70">
