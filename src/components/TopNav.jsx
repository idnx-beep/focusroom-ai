...
export function TopNav({ activePanel, setActivePanel, onHome, onOpenLogin }) {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-30 px-4 py-4 sm:px-6 lg:px-8">
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-3 rounded-full border border-white/15 bg-black/20 px-3 py-3 shadow-glass backdrop-blur-2xl sm:px-5">
        <button onClick={onHome} className="flex min-w-0 items-center gap-2 rounded-full px-2 text-left text-white transition hover:text-sky-100">
          ...
          <span className="truncate text-sm font-semibold tracking-wide sm:text-base">FocusRoom AI</span>
        </button>
        ...
        <button onClick={onOpenLogin} className="focus-button px-3 py-2.5 sm:px-4" aria-label="登录 Demo">
