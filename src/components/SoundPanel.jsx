...
export function SoundPanel({ scene, sound, setSound, isAudioOn, onPreviewAudio, onStopAudio }) {
  ...
  return (
    <div className="min-w-0 space-y-4 sm:space-y-6">
      <div className="min-w-0 rounded-3xl border border-white/12 bg-white/[0.07] p-3.5 sm:p-4">
        <div className="flex items-center justify-between gap-4"><div className="min-w-0"><p className="text-sm text-white/54">当前音乐</p><p className="mt-1 truncate text-lg font-semibold text-white sm:text-xl">{currentMusic.label}</p></div><Headphones className="shrink-0 text-sky-100" /></div>
      ...
      <div className="min-w-0 rounded-3xl border border-white/12 bg-white/[0.07] p-3.5 sm:p-4">
        <div className="flex items-center justify-between gap-4"><div className="min-w-0"><p className="text-sm text-white/54">当前背景音</p><p className="mt-1 truncate text-lg font-semibold text-white sm:text-xl">{currentAmbience.label}</p></div><Volume2 className="shrink-0 text-sky-100" /></div>
        <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-white/12 bg-black/20 p-3 sm:p-4">
        <div className="min-w-0"><p className="text-sm text-white/54">声音状态</p><p className="mt-1 break-words font-semibold text-white">{isAudioOn ? "正在播放本地音乐和背景声" : "尚未启动，点击试听或开始学习"}</p></div>
        <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap"><button onClick={onPreviewAudio} className="focus-button primary-button px-4 py-2.5"><Play size={16} />试听声音</button><button onClick={onStopAudio} className="focus-button px-4 py-2.5"><Pause size={16} />停止</button></div>
      </div>
    </div>
  );
}
